    ...
    def __init__(self, config, 
        ...
        if not self.is_decoder:
            if self.use_img_trans:
                assert vision_model_config is not None, f"`use_img_trans` is set to True, the `vision_model_config` is required"
                self.img_transformer = SwinTransformer(**vision_model_config)
                
            # Some global variables
            visual_feature_dim = 1536 # Swin Large
