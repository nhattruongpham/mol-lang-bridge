...
def get_dataloader(dataset, batchsize, rank, world_size):
    ...
    def collate(batch):
        ...
        return (
            torch.concat(toked_smis, dim=0),
            torch.concat(desc_states, dim=0),
            torch.concat(desc_mask, dim=0),
            torch.concat(corrupted_toked_smis, dim=0),
        )
...
class ChEBIdataset(Dataset):
                    ...
                    if self.replace_desc:
                        ...
                        for token in doc:
                            if token.text == "is":
                                ...
                                break
                    res.append((int(line[0]), line[1].strip(), desc))
        return res
    ...
    def __getitem__(self, idx):
        ...
        dic["corrupted_toked_smis"] = (
            self.smi_tokenizer.corrupt(dic["smiles"])
