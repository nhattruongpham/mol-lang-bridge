...
def get_tokenizer():
    ...
    selfies_dict_list = [
        line.strip()
        for line in open(
            os.path.join(__file__.split("biot5/utils")[0], args.molecule_dict)
        )
    ]
...
class regexTokenizer:
    def __init__(self, path="../../datasets/SMILES/generate_vocab.txt", max_len=256):
        ...
        self.idtotok.update({0: "[PAD]", 1: "[SOS]", 2: "[EOS]"})
...
class SimpleSmilesTokenizer:
    def __init__(self, dir=None, smiles_vocab=None, max_len=256):
        self.smiles_vocab = (
            smiles_vocab if smiles_vocab is not None else build_simple_smiles_vocab(dir)
        )
        self.max_len = max_len
